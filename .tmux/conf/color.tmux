...
#
set-window-option -g status-right "#{prefix_highlight} #[bg=colour=236 fg=colour244]#S:#I.#P #[bg=colour=236 fg=colour231]#{online_status} #(~/.tmux/conf/scripts/status-right.sh \"[#{sysstat_cpu}:#{sysstat_mem}:#(df -h | grep -w '/' | tr -s ' ' | cut -d ' ' -f 5)]\")"

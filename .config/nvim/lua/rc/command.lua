...
vim.api.nvim_add_user_command('DiffOrig',
...
-- Change indent
vim.api.nvim_add_user_command('IndentChange', 'set tabstop=<args> shiftwidth=<args>',
                              {force = true, nargs = 1})

vim.api.nvim_add_user_command('EncodingReload', "execute 'e ++enc=<args>'",
                              {force = true, nargs = 1})
...
-- count word
vim.api.nvim_add_user_command('CountWord', "lua SetCmdLine(':%s/\\<<C-r><C-w>\\>/&/gn')",
                              {force = true})
...
-- Trim whitespace
vim.api.nvim_add_user_command('TrimWhiteSpace', "keeppatterns %s/\\s\\+$//e <Bar> :nohlsearch",
                              {force = true})
...
vim.api.nvim_add_user_command('FileWithNumber e', "echo join([expand('%'),  line('.')], ':')",
                              {force = true, nargs = 1})
...
vim.api.nvim_add_user_command('EditPluginConfigVim', function()
  ...
  vim.cmd(
      'edit ' .. vim.fn.resolve(vim.fn.expand(vim.fn.stdpath('config') .. '/rc/pluginconfig/')) ..
          '/' .. vim.fn.fnamemodify(plugin_name, ":r") .. '.vim')
end, {force = true})
vim.api.nvim_add_user_command('EditPluginConfigLua', function()
  ...
  vim.cmd('edit ' ..
              ...
              '/' .. vim.fn.fnamemodify(plugin_name, ":r") .. '.lua')
end, {force = true})

-- profile
vim.api.nvim_add_user_command('Profile',
...
vim.api.nvim_add_user_command('DeleteEmptyBuffers', function()
  for _, buf in ipairs(vim.fn.range(1, vim.fn.bufnr('$'))) do
    if vim.fn.buflisted(buf) ~= 0 and vim.fn.empty(vim.fn.bufname(buf)) == 1 and
        vim.fn.bufwinnr(buf) < 0 and vim.fn.getbufvar(buf, "&mod") == 0 then
      vim.cmd('bwipeout! ' .. buf)
    end
  end
end, {force = true})

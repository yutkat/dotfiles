...
vim.keymap.set('n', 'qq', function() return vim.fn.reg_recording() == '' and "qq" or "q" end,
...
-- move cursor
vim.keymap.set({'n', 'x'}, 'j', function() return vim.v.count and "j" or "gj" end,
               {noremap = true, expr = true})
vim.keymap.set({'n', 'x'}, 'k', function() return vim.v.count and "k" or "gk" end,
               {noremap = true, expr = true})
...
if vim.fn.maparg('<F5>', 'n') == '' then
  vim.keymap.set('n', '<F5>', ":<C-u>nohlsearch<C-r>=has('diff')?'<Bar>diffupdate':''<CR><CR><C-l>",
...
vim.keymap.set('n', '<F3>',
               ...
               {noremap = true, silent = true})
vim.keymap.set('n', 'H', '<Cmd>if vimrc#is_normal_buffer() <Bar> execute "bprev" <Bar> endif <CR>',
               {noremap = true, silent = true})
vim.keymap.set('n', 'L', '<Cmd>if vimrc#is_normal_buffer() <Bar> execute "bnext" <Bar> endif <CR>',
               {noremap = true, silent = true})
...
-- For replace
vim.keymap
    .set('n', 'gr', 'gd[{V%::s/<C-R>///gc<left><left><left>', {noremap = true, silent = false})
...
vim.keymap
    .set('n', '<SubLeader>ej', '<Cmd>e ++enc=iso-2022-jp<CR>', {noremap = true, silent = true})

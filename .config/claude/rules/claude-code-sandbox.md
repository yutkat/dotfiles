...
- `~/.cache` is read-only in the sandbox, so tools that write caches there (mise, shuck) fail with "Read-only file system"; rerun with `XDG_CACHE_HOME=$TMPDIR/cache` instead of treating it as a code failure
